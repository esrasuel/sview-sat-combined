...
import datasets_satellite_into_sview as datasets
...
parser.add_argument("--mode", "-m", help="Training format >> 0:test, 1:train, 2:fine_tune_test, 3: fine_tune_train, 4: print test examples", default=1, type=int, choices=[0,1,2,3,4])
...
parser.add_argument("--satpatch", help="size of the satellite patches to extract", default=50, type=int)
...
print('Satellite file name...{}'.format(sat_file))
print('Satellite patch size...{}'.format(sat_patch))
print('Constraint label name...{}'.format(clabel_name))
...
if validation_flag == 0: # meaning we are doing cross validation
    DS = datasets.Dataset_CrossValidation(img_hdf5_file, lab_pickle_file, label_name, sat_file, sat_patch, clabel_name=clabel_name)
    ...
    DS = datasets.Dataset_TVT(img_hdf5_file, lab_pickle_file, label_name, sat_file, sat_patch, clabel_name=clabel_name)
    ...
    DS = datasets.Dataset_TT(img_hdf5_file, lab_pickle_file, label_name, sat_file, sat_patch, clabel_name=clabel_name)
    ...
    DS = datasets.Dataset_TT_byclass(img_hdf5_file, lab_pickle_file, label_name, sat_file, sat_patch, clabel_name=clabel_name, label_test=label_test)
    DS.pick_label(part_gen, part_file, train_part, psize=train_size)
...
class Net(nn.Module):
    def __init__(self, n_in, n_out, sat_psize, sat_channel): 
        ...
        self.dense6 = nn.Linear(64, n_out)

        # vgg part
        self.vgg_conv = nn.Sequential(
            nn.Conv2d(sat_channel, 16, 3, padding=1),
            nn.ReLU(),
        ...
        self.d11 = nn.Linear(csize**2*128, 512)
        self.d12 = nn.Linear(512, 256)
        self.d13 = nn.Linear(256, 64)

    def forward(self, x1, x2, x3, x4, xsat): 
        # first layer
        ...
        # processing satellite images now. 
        hsat = self.vgg_conv(xsat)
        
        hsat = torch.flatten(hsat, start_dim=1)
        hsat = F.relu(self.d11(hsat))
        hsat = F.relu(self.d12(hsat))
        hsat = self.d13(hsat)
        # out layer
        h = torch.cat([x, hsat], dim=1)
        h = F.relu(self.dense5(h))
        out = self.dense6(h)
        return convert2prob(out), out

net = Net(4096, 1, sat_patch*2, 4) #satellite patch is double sided
net.cuda()
...
if not os.path.exists("../../models/merge-sat-into-sview"):
    os.makedirs("../../models/merge-sat-into-sview")

if not os.path.exists("../../analysis"):
    os.makedirs("../../analysis")

if not os.path.exists("../../analysis/merge-sat-into-sview"):
    os.makedirs("../../analysis/merge-sat-into-sview")
...
if validation_flag == 0:
    ...
    save_name = '../../models/merge-sat-into-sview/{}_{}_out_of_{}_fold'.format(out_pre, part_kp, part_kn)
elif validation_flag == 1:
    log_file = '../log_dirs/logs_{}_division_tr{}_vl{}_te{}'.format(out_pre, train_part, validation_part, test_part)
    save_name = '../../models/merge-sat-into-sview/{}_division_tr{}_vl{}_te{}'.format(out_pre, train_part, validation_part,
                                                                                                             test_part)
elif validation_flag == 2:
    log_file = '../log_dirs/logs_{}_division_tr{}_te{}'.format(out_pre, train_part, test_part)
    save_name = '../../models/merge-sat-into-sview/{}_division_tr{}_te{}'.format(out_pre, train_part, test_part)

elif validation_flag == 3:
    log_file = '../log_dirs/logs_{}_division_classlabel'.format(out_pre)
    save_name = '../../models/merge-sat-into-sview/{}_division_classlabel'.format(out_pre)
...
if TRAIN or RTRAIN:
    ...
    for i, valid_data in enumerate(DS.validation_iterator(batch_size)): 
        ...
        xsat = torch.from_numpy(valid_data[4].transpose(0,3,1,2)).float().to(device)
        labels = torch.from_numpy(valid_data[5]).float().to(device)
        preds = net(xsvi1, xsvi2, xsvi3, xsvi4, xsat)[0].detach().cpu().numpy()
        cxloss += np.mean(-np.sum(valid_data[5] * np.log(preds + 1e-8), axis=1))
        mae_error += np.mean(np.abs(np.argmax(preds, axis=1) - np.argmax(valid_data[5], axis=1)))
        runi += 1
    ...
    for epoch in range(0,num_epochs): 
        ...
        for step in range(iter_per_epoch):
            ...
            xsat = torch.from_numpy(batch[4].transpose(0,3,1,2)).float().to(device)
            labels = torch.from_numpy(batch[5]).float().to(device)
            preds = net(xsvi1, xsvi2, xsvi3, xsvi4, xsat)[0]
        ...
        if epoch >= 0: 
            if np.mean(mae_error_list[-1]) < loss_min:
                ...
                torch.save(net.state_dict(), save_name + '_sviewNsat_network.pth')



    # writing down the mae error in the validation set over the epochs
    mae_error_name = '../../analysis/merge-sat-into-sview/{}_mae_error_list.txt'.format(out_pre)
    np.savetxt(mae_error_name, mae_error_list)
    xent_error_name = '../../analysis/merge-sat-into-sview/{}_xentropy_error_list.txt'.format(out_pre)
    np.savetxt(xent_error_name, cxloss_list)
elif TEST or RTEST:
    print('Loading: ', save_name + '_sviewNsat_network.pth')
    net.load_state_dict(torch.load(save_name + '_sviewNsat_network.pth'))
    net.eval()
    # compute the MAE error before training...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(device)
    ...
    # == assigning the file names == #
    if validation_flag == 0:
        fname_h5 = '../../analysis/merge-sat-into-sview/{}_{}_out_of_{}_folds_h5_vals'.format(out_pre,part_kp, part_kn)
        fname_pred = '../../analysis/merge-sat-into-sview/{}_{}_out_of_{}_folds_predictions'.format(out_pre, part_kp, part_kn)
    elif validation_flag == 1:
        fname_h5 = '../../analysis/merge-sat-into-sview/{}_division_tr{}_vl{}_te{}_h5_vals'.format(out_pre, train_part, validation_part, test_part)
        fname_pred = '../../analysis/merge-sat-into-sview/{}_division_tr{}_vl{}_te{}_predictions'.format(out_pre, train_part, validation_part, test_part)
    elif validation_flag == 2:
        fname_h5 = '../../analysis/merge-sat-into-sview/{}_division_tr{}_te{}_h5_vals'.format(out_pre, train_part, test_part)
        fname_pred = '../../analysis/merge-sat-into-sview/{}_division_tr{}_te{}_predictions'.format(out_pre, train_part, test_part)
    elif validation_flag == 3:
        fname_h5 = '../../analysis/merge-sat-into-sview/{}_division_classlabel_h5_vals'.format(out_pre)
        fname_pred = '../../analysis/merge-sat-into-sview/{}_division_classlabel_predictions'.format(out_pre)
    ...
    for i, test_data_ in enumerate(DS.test_iterator(batch_size)): 
        ...
        xsat = torch.from_numpy(test_data[4].transpose(0,3,1,2)).float().to(device)
        labels = torch.from_numpy(test_data[5]).float().to(device)
        preds = net(xsvi1, xsvi2, xsvi3, xsvi4, xsat)
        preds_ = preds[0].detach().cpu().numpy()
        h5_ = preds[1].detach().cpu().numpy()
        mae += np.sum(np.abs(np.argmax(preds_, axis=1) - np.argmax(test_data[5], axis=1)))
        preds_list.append(preds_)
        h5_list.append(h5_)
        vmap_list.append(np.concatenate([vmap, h5_, np.argmax(preds_, axis=1)[:,None], np.argmax(test_data[5], axis=1)[:,None]], axis=1))
        runi += batch_size
        if i % 500 == 499:
            print(mae / runi)
    ...
    df = pd.DataFrame(np.concatenate(vmap_list, axis=0), columns=['img_id', 'pcd', 'oa11', 'lsoa11', 'h5', 'predicted', DS.label_name])
