...
import datasets_satellite as datasets
...
if validation_flag == 0: # meaning we are doing cross validation
    DS = datasets.Dataset_CrossValidation(sat_hdf5_file, sat_hdf5_labfile, label_name, sat_patch, label_set)
    ...
    DS = datasets.Dataset_TVT(sat_hdf5_file, sat_hdf5_labfile, label_name, sat_patch, label_set)
    ...
    DS = datasets.Dataset_TT(sat_hdf5_file, sat_hdf5_labfile, label_name, sat_patch, label_set)
    ...
    DS = datasets.Dataset_TT_byclass(sat_hdf5_file, sat_hdf5_labfile, label_name, sat_patch, label_set, label_test=label_test)
    DS.pick_label(part_gen, part_file, train_part, psize=train_size)
...
class UNet_Tiny(nn.Module): 
    def __init__(self, n_in, n_out): 
        ...
        self.dec_c1 = nn.Conv2d(128, 64, 3, padding=1)
        # concatenation happens here
        self.dec_c2 = nn.Conv2d(64+64, 64, 3, padding=1)
        self.dec_c3 = nn.Conv2d(64, 64, 3, padding=1)
        self.dec_c4 = nn.Conv2d(64, n_out, 3, padding=1)

    def forward(self, x): 
        x = F.relu(self.enc_c1(x))
        x_to_connect = F.relu(self.enc_c2(x))
        x = self.pool(x_to_connect)
        x = F.relu(self.enc_c3(x))
        x = F.relu(self.enc_c4(x))
        x = self.up(x)
        x = F.relu(self.dec_c1(x))
        x = torch.cat([x, x_to_connect], dim=1)
        x = F.relu(self.dec_c2(x))
        x = F.relu(self.dec_c3(x))
        out = self.dec_c4(x)
...
optimizer = optim.Adam(UNet.parameters(), lr=lrrate, weight_decay=0.0001)
...
if not os.path.exists("../../models/sat-only"):
    os.makedirs("../../models/sat-only")

if not os.path.exists("../../analysis"):
    os.makedirs("../../analysis")

if not os.path.exists("../../analysis/sat-only"):
    os.makedirs("../../analysis/sat-only")
...
if validation_flag == 0:
    ...
    save_name = '../../models/sat-only/{}_{}_out_of_{}_fold'.format(out_pre, part_kp, part_kn)
elif validation_flag == 1:
    log_file = '../log_dirs/logs_{}_division_tr{}_vl{}_te{}'.format(out_pre, train_part, validation_part, test_part)
    save_name = '../../models/sat-only/{}_division_tr{}_vl{}_te{}'.format(out_pre, train_part, validation_part,
                                                                                                             test_part)
elif validation_flag == 2:
    log_file = '../log_dirs/logs_{}_division_tr{}_te{}'.format(out_pre, train_part, test_part)
    save_name = '../../models/sat-only/{}_division_tr{}_te{}'.format(out_pre, train_part, test_part)
elif validation_flag == 3:
    log_file = '../log_dirs/logs_{}_division_classlabel'.format(out_pre)
    save_name = '../../models/sat-only/{}_division_classlabel'.format(out_pre)
...
if TRAIN or RTRAIN: 
    ...
    while run_num > 0:
        for i, valid_data in enumerate(DS.validation_iterator(psat_size=1000)): 
            x_sat = torch.from_numpy(valid_data[0].transpose(0,3,1,2)).float().to(device)
            preds_ = UNet(x_sat)[0].to('cpu').detach().numpy()
            mae_error += np.sum(np.abs( np.argmax( preds_, axis=1 ) - np.argmax( valid_data[1], axis=3 ) ) * np.max(valid_data[1], axis=3)) / np.sum(valid_data[1])
            cxloss += np.mean(-np.sum(valid_data[1].transpose(0,3,1,2) * np.log(preds_ + 1e-8), 1))
            runi += 1
        run_num = run_num - 1
    ...
    for epoch in range(0, num_epochs): 
        ...
        for step in range(0, 500): 
            ...
            x_sat = torch.from_numpy(batch[0].transpose(0,3,1,2)).float().to(device)
            labels = torch.from_numpy(batch[1].transpose(0,3,1,2)).float().to(device)
    ...
    mae_error_name = '../../analysis/sat-only/{}_mae_error_list.txt'.format(out_pre)
    np.savetxt(mae_error_name, mae_error_list)
    xent_error_name = '../../analysis/sat-only/{}_xentropy_error_list.txt'.format(out_pre)
    np.savetxt(xent_error_name, cxloss_list)

else: # meaning we are testing the algorithms
    print('Loading: ', save_name + '_unet.pth')
    ...
    if validation_flag == 0:
        fname = '../../analysis/sat-only/{}_{}_out_of_{}_folds'.format(out_pre,part_kp, part_kn)
        ...
        fname = '../../analysis/sat-only/{}_division_tr{}_vl{}_te{}'.format(out_pre, train_part, validation_part, test_part)
        fname_h5 = fname + '_h5_vals'
        fname_pred = fname + '_predictions'
        fname_image = fname + '_image'
        fname_lsoa = fname + '_lsoa'
        ...
        fname = '../../analysis/sat-only/{}_division_tr{}_te{}'.format(out_pre, train_part, test_part)
        fname_h5 = fname + '_h5_vals'
        fname_pred = fname + '_predictions'
    elif validation_flag == 3:
        fname = '../../analysis/sat-only/{}_division_classlabel'.format(out_pre)
        fname_h5 = fname + '_h5_vals'
        fname_pred = fname + '_predictions'
    ...
    def aggregate_mae(labels, preds, h5s, lsoas): 
        ...
        return mae / lsoa_list.size, preds_lsoa_list, h5_lsoa_list, label_lsoa_list, lsoa_list_to_write
    ...
    for i, test_data_ in enumerate(DS.test_iterator(psat_size=1000)):
        ...
        x_sat = torch.from_numpy(test_data[0].transpose(0,3,1,2)).float().to(device)
        preds, h5 = UNet(x_sat)
        preds_ = preds.to('cpu').detach().numpy()
        h5_ = h5.to('cpu').detach().numpy()[0,...]
        mae += np.sum(np.abs( np.argmax( preds_, axis=1 ) - np.argmax( test_data[1], axis=3 ) ) * np.max(test_data[1], axis=3)) / np.sum(test_data[1])
        mae_lsoa_, pred_list, h5_list, label_list, lsoa_list_to_write = aggregate_mae(np.argmax(test_data[1], axis=3), np.argmax(preds_, axis=1), h5_, lsoa_info)
        mae_lsoa += mae_lsoa_
        ...
        tiff.imsave(tiff_file, np.argmax(test_data[1], axis=3)[0,...].astype(np.float))
        tiff_file = fname_image + '_{}.tif'.format(i)
        img_ = test_data[0][0,:,:,1:4]
        m = img_.min()
        M = img_.max()
