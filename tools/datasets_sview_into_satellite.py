...
class Dataset:
    def __init__(self,
        ...
        self.labf_sview = pd.read_csv(sview_lab_file)
        self.labels_sview = self.labf_sview[sview_label_name].to_numpy()
        ...
        self.sview_label_name = sview_label_name
    ...
    def get_data_part(self, rows, noise_std=None, lx_=None, ly_=None, psat_size=None):
        ...
        if (lx_ is None) and (ly_ is None):
            ...
            lx = lx_
            ly = ly_
        img_ = self.satims[srows, lx:lx+psat_size_, ly:ly+psat_size_, :][None,:,:,:]
        img_[:,:,:,:4] = normalize_satellite(img_[:,:,:,:4])
        # getting data from the street view images
        # number of sview images in the patch
        nsview = np.sum(img_[:,:,:,4] >= 0)
        # locate these points and get their sview ids.
        wsview = np.where(img_[:,:,:,4] >= 0)
        sview_rows = img_[wsview[0], wsview[1], wsview[2], 4]
        # sort the sview ids but do not forget the original order.
        argsort_sview_rows = np.argsort(sview_rows)
        sorted_sview_rows = sview_rows[argsort_sview_rows]
        # get the codes and place the values into their original order when
        # creating the d1, d2, d3 and d4.
        d1 = np.zeros((nsview, self.codes.shape[2]))
        d1[argsort_sview_rows,:] = normalize_features(np.squeeze(self.codes[sorted_sview_rows, 0, :]))
        d2 = np.zeros((nsview, self.codes.shape[2]))
        d2[argsort_sview_rows,:] = normalize_features(np.squeeze(self.codes[sorted_sview_rows, 1, :]))
        d3 = np.zeros((nsview, self.codes.shape[2]))
        d3[argsort_sview_rows,:] = normalize_features(np.squeeze(self.codes[sorted_sview_rows, 2, :]))
        d4 = np.zeros((nsview, self.codes.shape[2]))
        d4[argsort_sview_rows,:] = normalize_features(np.squeeze(self.codes[sorted_sview_rows, 3, :]))

        # getting the labels exactly the same way.
        labs = self.labels[srows, lx:lx+psat_size_, ly:ly+psat_size_][:, :].astype(np.int)
        l = [] 
        for j in range(self.label_num): 
            l_ = np.zeros([psat_size_, psat_size_])
            l_[labs == self.label_set[j]] = 1.0
            l = l + [l_]
        l = np.asarray(l).transpose(1,2,0).astype(np.float32)[None,:,:,:]
        ...
        # also getting the labels at the sview locations for pre-training.
        lsview = l[wsview[0],wsview[1], wsview[2], :]
        lsview_ = np.zeros(lsview.shape[0])
        lsview_[argsort_sview_rows] = self.labels_sview[sorted_sview_rows.astype(np.int)]
        return d1, d2, d3, d4, img, l, lsview
    ...
    def get_train_batch(self, batch_size):
        ...
        d1, d2, d3, d4, img, l, lsview = self.get_data_part(rows)
    ...
    def get_validation_batch(self, batch_size, lx_=None, ly_=None):
        rows = self.validation_part[self.batch_ind_valid: self.batch_ind_valid + batch_size]
        d1, d2, d3, d4, img, l, lsview = self.get_data_part(rows,lx_=lx_, ly_=ly_)
...
class Dataset_TT(Dataset):
    ...
    def validation_iterator(self, batch_num=1):
        return self.training_iterator(batch_num=batch_num)
