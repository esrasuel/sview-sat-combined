...
def decimate_partition_stratified(current_partition, labels, psize=1.0, clabels=None):
    if clabels is None:
        train_part, other_part = partition_stratified(labels[current_partition], train_size=psize)
    else: 
        train_part, other_part = partition_stratified(labels[current_partition], train_size=psize,
                                                    clabels=clabels[current_partition])
    return list(np.asarray(current_partition)[train_part]), \
        list(np.asarray(current_partition)[other_part])


def decimate_partition(current_partition, psize=1.0):
    train_part, other_part = partition(np.asarray(current_partition), train_size=psize)
    return list(np.asarray(current_partition)[train_part]), \
        list(np.asarray(current_partition)[other_part])
...
def partition(labels, train_size, seed=None):
    ...
    test_partition = []
    np.random.seed(seed)
    rows = np.random.permutation(labels.shape[0])
    lab_size = labels.shape[0]
    row_end = np.int(lab_size * train_size)
    train_partition += list(rows[:row_end])
    row_beg = row_end
    test_partition += list(rows[row_beg:])
